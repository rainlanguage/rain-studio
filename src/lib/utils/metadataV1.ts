 ...
 */
export const cborDecode = (dataEncoded_: string): Array<Map<number, any>> => {
 ...
 */
export const decodeRainDocument = (dataEncoded_: string) => {
	const metaDocumentHex = '0x' + MAGIC_NUMBERS.RAIN_META_DOCUMENT.toString(16).toLowerCase();

	dataEncoded_ = dataEncoded_.toLowerCase().startsWith('0x') ? dataEncoded_ : '0x' + dataEncoded_;

	if (!dataEncoded_.startsWith(metaDocumentHex)) {
		throw new Error('Invalid data. Does not start with meta document magic number.');
	}
