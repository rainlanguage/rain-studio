 ...
 */
export const matchInterpreters = async (addresses: string[], supabaseClient?: TypedSupabaseClient) => {
    const interpreters_array = addresses
    if (!supabaseClient) supabaseClient = _supabaseClient
    const interpretersQuery = await supabaseClient.rpc('match_interpreters_by_address', { interpreters_array })
...
export const matchContracts = async (addresses: string[], supabaseClient?: TypedSupabaseClient) => {
    ...
    const contractsQuery = await supabaseClient.rpc('match_contracts_by_address', { contracts_array })
