...
export const load: PageServerLoad = async (event) => {
    ...
    if (contractQuery.error) throw sveltekitError(404, 'Not found');

    const interpretersQuery = await supabaseClient.from('interpreters').select('*');
    if (interpretersQuery.error) throw sveltekitError(500, 'Something went wrong :(');
    ...
    const accountsQuery = await supabaseClient
        .from('wallets_linked')
        .select('address, user_id(username, avatar_url)')
        .in('address', _accounts);

    accountsQuery.data.forEach((account_) => {
        _accountsData[account_.address.toLowerCase()] = {
            username: account_.user_id.username,
            avatar_url: account_.user_id.avatar_url
        };
    });
